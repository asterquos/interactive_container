...
class Container:
    ...
    def __init__(self, length: float = None, width: float = None, name: str = "Container"):
        """初始化集装箱"""
    ...
    def find_placement_position(self, box: Box) -> Optional[Tuple[float, float]]:
        ...
        for y in range(0, int(self.width - box.actual_width) + step, step):
            for x in range(0, int(self.length - box.actual_length) + step, step):
                box.move_to(x, y)
                if self.can_place_box(box):
                    return (x, y)
